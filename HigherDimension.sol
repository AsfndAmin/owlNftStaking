...
contract HigherDimensions is ERC721Upgradeable {
    // Royalty
    ...
    // map id to HigherDimension obj
    mapping(uint256 => HigherDimension) public allHigherDimension;
    ...
    event SaleToggle(uint256 hdNumber, bool isForSale, uint256 price);
    event PurchaseEvent(uint256 hdNumber, address from, address to, uint256 price);
    ...
    function initialize(
        ...
        __ERC721_init("Higher Dimensions", "HD");
    ...
    function mint(uint256 numberOfToken) public payable {
        ...
        for (uint256 i = 1; i <= numberOfToken; i++) {
            ...
            HigherDimension memory newHigherDimension = HigherDimension(
                newItemId,
                msg.sender,
                msg.sender,
                0,
            ...
            // add the token id to the allHigherDimension
            allHigherDimension[newItemId] = newHigherDimension;
        }
    }
    ...
    function setPriceForSale(
        ...
        HigherDimension memory hd = allHigherDimension[_tokenId];
        hd.price = _newPrice;
        hd.forSale = isForSale;
        allHigherDimension[_tokenId] = hd;
    ...
    function getAllSaleTokens() public view returns (uint256[] memory) {
        ...
        for (uint256 i = 1; i <= _totalSupply; i++) {
            if (allHigherDimension[i].forSale == true) {
                _tokenForSales[counter] = allHigherDimension[i].tokenId;
                counter++;
            }
        }
    ...
    function buyToken(uint256 _tokenId) public payable {
        ...
        // get that token from all HigherDimension mapping and create a memory of it defined as (struct => HigherDimension)
        HigherDimension memory hd = allHigherDimension[_tokenId];
        // price sent in to buy should be equal to or more than the token's price
        require(msg.value >= hd.price);
        ...
        payable(hd.currentOwner).transfer(payOwnerAmount);
        hd.previousPrice = hd.price;
        allHigherDimension[_tokenId] = hd;
        _transfer(tokenOwner, msg.sender, _tokenId);
        emit PurchaseEvent(_tokenId, hd.currentOwner, msg.sender, hd.price);
    }
    ...
    function _beforeTokenTransfer(
        ...
        HigherDimension memory hd = allHigherDimension[tokenId];
        hd.currentOwner = to;
        hd.numberOfTransfers += 1;
        hd.forSale = false;
        allHigherDimension[tokenId] = hd;
        if (from == address(0)) {
